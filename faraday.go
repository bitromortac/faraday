...
func (f *Faraday) Start() error {
	...
	// subservers.
	// TODO: we need to heal this connection on disconnects.
	var err error
	...
	// Initialize faraday with its dependencies.
...
func (f *Faraday) startRPCServer() error {
	...
	unaryInterceptor, streamInterceptor, err := f.macaroonService.Interceptors()
	if err != nil {
...
func stopRPCServer(f *Faraday) error {
	if f.restServer != nil {
		f.restCancel()
...
func (f *Faraday) StartAsSubserver(lndGrpc *lndclient.GrpcLndServices,
	...
	// Create the RPC rpcServer, but don't start it.
	f.RPCServer = frdrpcserver.NewRPCServer(cfg)
...
func (f *Faraday) Stop() error {
	...
	if f.macaroonService != nil {
		...
		if err != nil {
			return fmt.Errorf("error stopping macaroon service: %v", err)
		}
		f.macaroonDB.Close()
	}
...
func (f *Faraday) initialize(withMacaroonService bool) error {
	...
	if f.cfg.ChainConn {
		// TODO: expose Disconnect and call on Stop.
