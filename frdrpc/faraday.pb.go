...
type ForwardingAbilityRequest struct {
	...
	// Start of query window (unix epoch seconds).
	StartTime uint64 `protobuf:"varint,1,opt,name=start_time,json=startTime,proto3" json:"start_time,omitempty"`
	// End of query window (unix epoch seconds).
	EndTime uint64 `protobuf:"varint,2,opt,name=end_time,json=endTime,proto3" json:"end_time,omitempty"`
	// Percentile of successful forward amounts (0-100) for the liquidity
	// floor. Combined with threshold_amt_sat as max(percentile_quantile,
	// threshold). Zero disables the percentile component.
	...
	// Manual lower bound on routable liquidity (satoshis). Zero disables the
	// manual component.
...
type ForwardingAbilityResponse struct {
	...
	// Sorted dictionary of distinct peer pubkeys that appear as either
	// incoming or outgoing peer in the matrix, encoded as raw 33-byte
	// compressed pubkeys (not hex). Sorted by raw-byte comparison so
	// identical inputs produce byte-identical responses.
	//
	// Acts as the index space for uptime_fractions and velocities: position
	// i in this list is the integer peer id i used in those fields. The
	// layout dimension P is len(peers).
	Peers [][]byte `protobuf:"bytes,1,rep,name=peers,proto3" json:"peers,omitempty"`
	// Dense uptime matrix in row-major order: pair (in, out) lives at byte
	// offset 2 * (in * P + out) and occupies two bytes encoded as a
	// little-endian uint16. The quantum q represents the fraction
	// q / 65535 in [0, 1]; the encoder applies math.Round before clamping.
	//
	// Length invariant: len(uptime_fractions) == 2 * P * P. Pairs that the
	// analyzer did not produce encode as zero (q = 0), which is
	// well-defined and indistinguishable from an explicit zero — uptime
	// has no "missing" semantic distinct from "zero".
	//
	// Worked example for P = 2 (peers a, b sorted as raw bytes):
	// bytes 0..1: uptime(a, a) little-endian uint16
	// bytes 2..3: uptime(a, b)
	// bytes 4..5: uptime(b, a)
	// bytes 6..7: uptime(b, b)
	UptimeFractions []byte `protobuf:"bytes,2,opt,name=uptime_fractions,json=uptimeFractions,proto3" json:"uptime_fractions,omitempty"`
	// Sparse list of pair velocities. A pair (in, out) appears here only
	// when its velocity is non-zero, so a healthy node with mostly idle
	// pairs sends very few entries even at large P.
	Velocities []*VelocityEntry `protobuf:"bytes,3,rep,name=velocities,proto3" json:"velocities,omitempty"`
}
...
func (x *ForwardingAbilityResponse) GetUptimeFractions() []byte {
	if x != nil {
		return x.UptimeFractions
	}
	return nil
}

func (x *ForwardingAbilityResponse) GetVelocities() []*VelocityEntry {
	if x != nil {
		return x.Velocities
	}
	return nil
...
type VelocityEntry struct {
	...
	// Packed peer index pair: the high 16 bits hold the incoming peer
	// index and the low 16 bits hold the outgoing peer index, both
	// indexing into ForwardingAbilityResponse.peers. The 16-bit per-side
	// width caps P at 65536, well above any realistic peer count.
	//
	// Layout (most significant bit first):
	// bits 31..16: in peer index
	// bits 15..0:  out peer index
	PackedIdx uint32 `protobuf:"varint,1,opt,name=packed_idx,json=packedIdx,proto3" json:"packed_idx,omitempty"`
	// Forwarding velocity in sat/s during effective uptime, encoded as
	// float32 because the wire-side precision (~7 significant digits) is
	// well above any downstream consumer's need and halves the per-entry
	// cost compared to double. +Inf indicates forwards were observed
	// without the pair ever crossing the liquidity threshold — a model
...
func (x *VelocityEntry) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*VelocityEntry) ProtoMessage() {}

func (x *VelocityEntry) ProtoReflect() protoreflect.Message {
	mi := &file_faraday_proto_msgTypes[27]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
...
// Deprecated: Use VelocityEntry.ProtoReflect.Descriptor instead.
func (*VelocityEntry) Descriptor() ([]byte, []int) {
	return file_faraday_proto_rawDescGZIP(), []int{27}
}

func (x *VelocityEntry) GetPackedIdx() uint32 {
	if x != nil {
		return x.PackedIdx
	}
	return 0
...
func (x *VelocityEntry) GetVelocity() float32 {
	if x != nil {
		return x.Velocity
	}
	return 0
...
var file_faraday_proto_rawDesc = []byte{
	...
	0x65, 0x12, 0x2d, 0x0a, 0x12, 0x66, 0x6f, 0x72, 0x77, 0x61, 0x72, 0x64, 0x5f, 0x70, 0x65, 0x72,
	0x63, 0x65, 0x6e, 0x74, 0x69, 0x6c, 0x65, 0x18, 0x03, 0x20, 0x01, 0x28, 0x02, 0x52, 0x11, 0x66,
	0x6f, 0x72, 0x77, 0x61, 0x72, 0x64, 0x50, 0x65, 0x72, 0x63, 0x65, 0x6e, 0x74, 0x69, 0x6c, 0x65,
	0x12, 0x2a, 0x0a, 0x11, 0x74, 0x68, 0x72, 0x65, 0x73, 0x68, 0x6f, 0x6c, 0x64, 0x5f, 0x61, 0x6d,
	0x74, 0x5f, 0x73, 0x61, 0x74, 0x18, 0x04, 0x20, 0x01, 0x28, 0x04, 0x52, 0x0f, 0x74, 0x68, 0x72,
	0x65, 0x73, 0x68, 0x6f, 0x6c, 0x64, 0x41, 0x6d, 0x74, 0x53, 0x61, 0x74, 0x22, 0x93, 0x01, 0x0a,
	...
	,
	...
	,
	...
	0x69, 0x6e, 0x67, 0x6c, 0x61, 0x62, 0x73, 0x2f, 0x66, 0x61, 0x72, 0x61, 0x64, 0x61, 0x79, 0x2f,
	0x66, 0x72, 0x64, 0x72, 0x70, 0x63, 0x62, 0x06, 0x70, 0x72, 0x6f, 0x74, 0x6f, 0x33,
}
...
var file_faraday_proto_goTypes = []any{
	...
	(*VelocityEntry)(nil),                   // 32: frdrpc.VelocityEntry
...
var file_faraday_proto_depIdxs = []int32{
	...
	32, // 21: frdrpc.ForwardingAbilityResponse.velocities:type_name -> frdrpc.VelocityEntry
