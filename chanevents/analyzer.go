...
func (a *ForwardingAnalyzer) getForwardingData(ctx context.Context, startTime,
	...
	for {
		...
		if len(fwds.Events) < maxForwardQueries {
			break
		}
...
func calculateAllPairsUptime(ctx context.Context, store EventsSource, startTime,
	...
	for i, peerA := range peers {
		...
		for j := i; j < len(peers); j++ {
			...
			inputsAB := pairThresholdInputs(
				liquidityFloor, successfulForwards, peerA, peerB,
			)
			inputsBA := pairThresholdInputs(
				liquidityFloor, successfulForwards, peerB, peerA,
			)
...
// amount for one direction of a peer pair. The floor is the single, uniform
// liquidityFloor applied to every pair.
func pairThresholdInputs(liquidityFloor btcutil.Amount,
	successfulForwards map[PeerPair][]btcutil.Amount,
	peerIn, peerOut string) pairInputs {
...
// accumulators, as both share the same uniform liquidity floor. For self-pair
// calls (statesA == statesB, inputsAB == inputsBA) both returned abilities are
// equal.
func calculateBothDirectionsUptime(ctx context.Context, startTime,
	...
	*ForwardingAbility, *ForwardingAbility, error) {
	...
	abilityAB := makeAbility(uptimeAB, inputsAB.totalSuccessfulAmount)
	abilityBA := makeAbility(uptimeBA, inputsBA.totalSuccessfulAmount)
