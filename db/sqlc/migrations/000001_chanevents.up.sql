...
CREATE TABLE IF NOT EXISTS peers (
    ...
    -- An optional alias for the peer.
    alias TEXT
);
...
CREATE TABLE IF NOT EXISTS channel_events (
    ...
    -- This is only populated for balance update events.
    remote_balance_sat BIGINT
);

-- This composite index is crucial for efficiently querying the event history
-- of a specific channel within a time range, as it allows the database to
-- quickly locate the relevant rows and retrieve them already sorted by
-- timestamp.
CREATE INDEX channel_events_chan_id_ts_idx ON channel_events (channel_id, timestamp);
